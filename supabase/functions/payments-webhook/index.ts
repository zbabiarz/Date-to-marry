...
async function handleCheckoutSessionCompleted(
  ...
  if (session.metadata?.token_purchase === "true" && session.amount_total) {
    try {
      ...
      // Call the update-tokens-on-purchase function
      const response = await fetch(
        `${Deno.env.get("SUPABASE_URL")}/functions/v1/update-tokens-on-purchase`,
        {
          method: "POST",
          headers: {
      ...
      return new Response(
        JSON.stringify({ message: "Tokens added successfully", result }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
